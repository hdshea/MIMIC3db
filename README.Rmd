...
1. you are in the base directory of an RStudio project that has a `data` directory defined in it, 
2. you have loaded you have loaded the SQL scripts from  [this GitHub directory](https://github.com/hdshea/MIMIC3db/tree/main/data) into the `data` directory, and
3. you have loaded the MIMIC-III v1.4 data files into the `data` directory
...
NOTE:  the zip file for the base data tables is 6.2 GB and the built database on my MacBook Pro M1 local 
hard drive is 85.6 GB.
...
```{r list_tables, include=TRUE}
RSQLite::dbListTables(con)
```

```{r pre_disconnect, include=FALSE}
...
`sata/MIMIC-III.db`, then the following code will set up a connection `con` to that database.
...
(NOTE:  The example database I use for testing has only empty tables.  That is why there are no results returned in the examples calss above.)
